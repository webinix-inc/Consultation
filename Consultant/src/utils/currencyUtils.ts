...
export const mapCountryToCode = (country: string): string => {
    if (!country) return '';
    const c = country.toLowerCase().trim();
    if (c === 'united states' || c === 'usa' || c === 'america' || c === 'us') return 'us';
...
export const getCurrencyConfig = (countryCode: string) => {
    ...
    const code = mapCountryToCode(countryCode) || countryCode;
    return currencyMap[code.toLowerCase()] || { symbol: '$', code: 'USD' };
};
