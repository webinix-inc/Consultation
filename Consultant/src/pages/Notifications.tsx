...
const NotificationsPage: React.FC = () => {
  ...
  const currentConsultant = useMemo(() => {
    ...
    if (usersData?.data) {
      ...
      if (found) return { _id: found._id || found.id, id: found._id || found.id, email: found.email, name: found.fullName || found.name };
    }
    return null;
  ...
  const linkedClients = useMemo(() => {
    if (!linkedClientsData) return [];
    if (Array.isArray(linkedClientsData)) return linkedClientsData;
    return [];
  }, [linkedClientsData]);
  ...
  const handleSendToClient = () => {
    if (!notificationForm.name || !notificationForm.message || !notificationForm.recipient) {
      return;
    }
    sendToClient({
  ...
  return (
    <motion.div
      className="min-h-screen bg-gray-50/50 p-6 space-y-8 relative"
      animate={{
    ...
    >
      ...
      {/* Send Notification to Client Dialog */}
      <Dialog open={sendModalOpen} onOpenChange={setSendModalOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Send Notification to Client</DialogTitle>
            <DialogDescription>
              Send a notification to one of your linked clients.
            </DialogDescription>
          </DialogHeader>
